...
EnergyslopeTau::EnergyslopeTau(bool correctionForBedloadWeightAtSteepSlopes):
	correctionForBedloadWeightAtSteepSlopes(correctionForBedloadWeightAtSteepSlopes)
{}

EnergyslopeTau::~EnergyslopeTau() {}

CalcTau* EnergyslopeTau::createCalcTauPointerCopy() const
{
	CalcTau* result = new EnergyslopeTau(this->correctionForBedloadWeightAtSteepSlopes);
	return result;
}

ConstructionVariables EnergyslopeTau::createConstructionVariables()const
{
	ConstructionVariables result = ConstructionVariables();
	result.interfaceOrCombinerType = CombinerVariables::CalcTau;
	result.realisationType = CombinerVariables::typeOfTauCalculationMethodToString(CombinerVariables::EnergyslopeTau);
...
double EnergyslopeTau::calculate (const RiverReachProperties& riverReachProperties, const FlowResistance* flowResistance) const
{
	...
	double hydraulicRadiusWithinActiveWidth = riverReachProperties.geometricalChannelBehaviour->alluviumChannel->getHydraulicRadiusForActiveWidth(riverReachProperties.regularRiverReachProperties.maximumWaterdepth, riverReachProperties.regularRiverReachProperties.activeWidth);

	return (hydraulicRadiusWithinActiveWidth * (riverReachProperties.getOverallParameters())->getWaterDensity() * (riverReachProperties.getOverallParameters())->getGravityAcceleration() * correctedSedimentEnergySlope);
}
